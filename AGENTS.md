...
- MacBook: the primary dev + verification machine. The Playwright MCP is registered for omp at `~/.omp/agent/mcp.json` (added 2026-07-11; new sessions pick it up, `/mcp reload` for a running one) and ships with Claude Code as a plugin (`browser_run_code_unsafe`). Fallback that always works: playwright-core scripts (pattern in `~/tmp/pw-qa/qa.mjs`, launching the ms-playwright cached "Google Chrome for Testing.app" binary). Either way pass `page.screenshot({ path })` an absolute path outside the repo — pathless screenshots land in the repo root and must be cleaned before committing. For FPS work, create the context with `deviceScaleFactor: 2`; `setViewportSize` alone leaves dpr at 1 and hides all fill-rate cost.
